...
function generateOffsetIdentifier(config) {
  return `${config.offsetFilePrefix ?? "offset"}_${config.name}`;
}
function generateCollectionName(config, queryConfig) {
  return `${config.datasourceIdentifier}_${queryConfig.name}`;
}
