...
class JsessionManager {
  constructor(authUrl, username, password) {
    ...
    this.getSessionCooky().then((cooky) => {
      if (cooky) {
  ...
  #sessionExpiresAt;
  #logger;
  async getSessionCooky() {
    this.#logger.debug("Getting session cooky");
    const now = Date.now();
    if (this.#sessionCooky && this.#sessionExpiresAt && now < this.#sessionExpiresAt) {
    ...
    try {
      ...
      this.#sessionCooky = response.headers["set-cookie"] ? response.headers["set-cookie"][0] : void 0;
