var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var redis_cache_service_exports = {};
__export(redis_cache_service_exports, {
  RedisCacheService: () => RedisCacheService
});
module.exports = __toCommonJS(redis_cache_service_exports);
var import_redis = require("redis");
class RedisCacheService {
  #redisClient;
  #options;
  constructor(redisClient, options = {}) {
    if (typeof redisClient === "string") {
      redisClient = (0, import_redis.createClient)({ url: redisClient });
      redisClient.connect();
    }
    this.#redisClient = redisClient;
    this.#options = options;
  }
  async get(key) {
    const value = await this.#redisClient.get(this.getKey(key));
    return value ? JSON.parse(value) : void 0;
  }
  async set(key, value) {
    await this.#redisClient.set(this.getKey(key), JSON.stringify(value));
  }
  async clear() {
    await this.#redisClient.flushDb();
  }
  getKey(key) {
    return `${this.#options.cacheKeyPrefix ? `${this.#options.cacheKeyPrefix}_` : ""}${key}`;
  }
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  RedisCacheService
});
//# sourceMappingURL=redis-cache.service.js.map
